...
export const Config = () => {
  ...
  const userConfig = async (e) => {
    ...
    // Verificamos si la informacion que nos llega es correcta
    if (data.status== "success" && fileInput.files[0]) {
        ...
        const dataForm = await upload.json();
        console.log("soy el formData",dataForm);
        if (dataForm.status == "success" && formData.user) {
          delete formData.user.password
          setAuth(dataForm.user)
          setSaved("success");
